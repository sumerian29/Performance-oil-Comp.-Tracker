...
# ---------------------------
st.set_page_config(
    page_title="Global Oil Companies Performance Tracker",  # Default title in English
    layout="wide",
    page_icon="🛢️"
)
...
if language == "Arabic":
    L = {
        "page_title": "متتبع أداء شركات النفط العالمية",
        "app_title": "متتبع أداء شركات النفط العالمية",
        "app_description": ("يقدم هذا التطبيق تحليلاً شاملاً لأداء شركات النفط العالمية على مدى السنوات الثلاث الماضية.\n\n"
        ...
        "num_companies": "عدد الشركات",
        "average_growth_rate": "متوسط معدل النمو",
        "top_growing_company": "الشركة ذات النمو الأعلى",
        "summary": "ملخص",
        "summary_text": ("يوفر هذا التطبيق نظرة شاملة على أداء شركات النفط العالمية على مدى السنوات الثلاث الماضية، "
    ...
    L = {
        "page_title": "Global Oil Companies Performance Tracker",
        "app_title": "Global Oil Companies Performance Tracker",
        "app_description": ("This application provides a comprehensive analysis of the performance of global oil companies over the past three years.\n\n"
        ...
        "top_growing_company": "Top Growing Company",
        "summary": "Summary",
        "summary_text": ("This application provides a comprehensive overview of global oil companies' performance over the past three years, "
...
def ml_forecast(data_dict, start_date, end_date):
    ...
    for ticker, df in data_dict.items():
        # Filter each company's data based on the selected date range
        df_ml = df.copy()
...
def main():
    ...
    # Full names of oil companies (30 companies)
    oil_companies_full = {
    ...
    tickers = list(oil_companies_full.keys())
    ...
    display_names = [f"{ticker} - {oil_companies_full.get(ticker, '')}" for ticker in available_tickers]
    ...
    st.subheader(f"{selected_ticker} - {oil_companies_full.get(selected_ticker, '')} {L['data_last_30']}")
    if not ticker_data_filtered.empty:
        st.dataframe(ticker_data_filtered.tail(30))
    else:
        st.info("No data available for the selected date range.")

    st.subheader(f"{selected_ticker} - {oil_companies_full.get(selected_ticker, '')} {L['stock_price']}")
    fig_line = px.line(
    ...
    with st.expander(L["ml_section"], expanded=False):
        ...
        if forecast_results:
            ...
            st.success(f"**{L['recommendation_buy']} {best_ticker} - {oil_companies_full.get(best_ticker, '')}** with a predicted growth of {best_growth:.2f}%.")
            st.error(f"**{L['recommendation_sell']} {worst_ticker} - {oil_companies_full.get(worst_ticker, '')}** with a predicted growth of {worst_growth:.2f}%.")
        else:
